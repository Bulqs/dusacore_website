...
// Import your new payable-specific actions
import { 
    getPaymentMethods, 
    initiatePayablePayment, 
    verifyPayablePaymentStatus 
...
interface PayablePaymentModalProps {
    payableId: string;
    trackingNumber: string; // Used just for display/context
    amount: number;
    currency: string;
    description: string;
...
const PayablePaymentModal: React.FC<PayablePaymentModalProps> = ({ 
    payableId,
    trackingNumber,
    amount,
    currency,
    ...
    const handlePaymentSelection = async (method: PaymentMethodDTO) => {
        ...
        try {
            // Base URLs setup - Routing specifically to the payable verification logic
            const targetUrl = `${window.location.origin}/paymentstatus/stripe/verify?id=${payableId}&type=payable`;
            const paystackTargetUrl = `${window.location.origin}/paymentstatus/verify?id=${payableId}&type=payable`;
            ...
            // 1. INITIATE PAYABLE PAYMENT
            const response = await initiatePayablePayment(
                payableId, 
                method.provider, 
                payload
            );
            
            console.log("=== PAYABLE SESSION RESPONSE ===", response);
            console.log(`Payable ID: ${payableId} | Gateway Txn ID: ${response.transactionId}`);
            ...
            if (flow === 'EMBEDDED' || (flow === 'MODAL' && method.provider.toLowerCase().includes('stripe'))) {
                ...
                if (response.authorizationUrl) {
                    ...
                    throw new Error("No redirect URL provided by the gateway.");
                }
            } 
            else if (flow === 'INSTANT' || flow === 'DIRECT') {
                // EXPLICIT INTERNAL VERIFICATION FOR INSTANT FLOWS (e.g. Wallet)
                console.log("Calling instant verification for Payable:", payableId);
                
                try {
                    const verifyRes = await verifyPayablePaymentStatus(payableId);
                    console.log("Instant Verification Result:", verifyRes);
                    window.location.href = `${payload.callbackUrl}&reference=${response.transactionId}`;
                } catch (verifyErr) {
                    console.error("Instant Verification Failed:", verifyErr);
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-5xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col md:flex-row h-[90vh] md:h-[80vh]">
                ...
                <div className="w-full md:w-5/12 bg-gradient-to-b from-gray-50 to-gray-100 p-6 md:p-10 border-b md:border-r border-gray-200 flex flex-col overflow-y-auto">
                    <div className="mb-8">
                        <h3 className="text-2xl font-black text-gray-900 tracking-tight">Invoice Summary</h3>
                        <div className="flex items-center gap-2 mt-2">
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tracking Code:</span>
                            <span className="text-xs font-mono font-bold bg-white px-2 py-0.5 rounded border border-gray-200 text-gray-700">{trackingNumber}</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 flex-1 flex flex-col relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-6 pb-4 border-b border-dashed border-gray-200">
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-xs font-bold text-rose-500 mt-1 bg-rose-50 px-2 py-0.5 rounded inline-block">Unpaid</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 mb-6">
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-gray-600 font-medium flex items-center gap-2">
                                    <FiCheckCircle className="text-rose-500 w-4 h-4"/> 
                                    {description || "Outstanding Payable"}
                                </span>
                                <span className="font-bold text-gray-800">{currency} {(amount || 0).toLocaleString()}</span>
                            </div>
                        </div>

                        <div className="mt-auto pt-4 border-t border-dashed border-gray-200">
                            <div className="flex justify-between items-center bg-gray-900 p-4 rounded-xl shadow-inner">
                                <div>
                                    <span className="text-white/60 font-bold uppercase text-[10px] tracking-wider block mb-0.5">Total Amount Due</span>
                    ...
                    {stripeSession && stripeClientSecret ? (
                        <div className="flex flex-col h-full animate-in slide-in-from-right-4 duration-300">
                            <div className="flex items-center gap-3 mb-8">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-gray-500">Complete your payment securely via Stripe</p>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                                ...
                                >
                                    <StripePaymentForm 
                                        bookingId={payableId} 
                                        // ENSURE URL INCLUDES type=payable FOR THE VERIFY PAGE
                                        returnUrl={`${window.location.origin}/paymentstatus/stripe/verify?id=${payableId}&type=payable&reference=${stripeSession.transactionId}`}
                        ...
                        <div className="flex flex-col h-full overflow-hidden animate-in fade-in duration-300">
                            <div className="mb-8 pr-12">
                                ...
                                <p className="text-sm font-medium text-gray-500">Choose your preferred gateway to settle this invoice securely.</p>
                            </div>
