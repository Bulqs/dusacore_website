...
// CRITICAL FIX: Imported the verifyConsolidationPaymentStatus action here
import { 
    getPaymentMethods, 
    initiateConsolidationPayment, 
    verifyConsolidationPaymentStatus 
} from '@/lib/user/transaction.actions'; 

import StripePaymentForm from '../checkout/StripePaymentForm';
import Image from 'next/image';

interface ConsolidationPaymentModalProps {
...
const ConsolidationPaymentModal: React.FC<ConsolidationPaymentModalProps> = ({ 
    bookingData, 
    customerEmail, 
    customerName,
    onClose 
    ...
    const handlePaymentSelection = async (method: PaymentMethodDTO) => {
        ...
        try {
            // Base URLs setup
            const targetUrl = `${window.location.origin}/paymentstatus/stripe/verify?id=${bookingData.trackingNumber}&type=consolidation`;
            const paystackTargetUrl = `${window.location.origin}/paymentstatus/verify?id=${bookingData.trackingNumber}&type=consolidation`;
            ...
            // 1. INITIATE PAYMENT
            const response = await initiateConsolidationPayment(
                bookingData.trackingNumber, 
                method.provider, 
                payload
            );
            
            // LOGGING PAYLOAD TO CONSOLE / TERMINAL
            console.log("=== PAYMENT SESSION RESPONSE PAYLOAD ===", response);
            console.log(`Original ID: ${bookingData.trackingNumber} | Consolidation ID: ${response.transactionId}`);
            ...
            if (flow === 'EMBEDDED' || (flow === 'MODAL' && method.provider.toLowerCase().includes('stripe'))) {
                ...
                // Paystack handles verification on the redirect page callback.
                ...
                // We pass the base booking ID + the new -CNSLDT- reference string
                console.log("Calling consolidation verification instantly for:", response.transactionId);
                
                try {
                    const verifyRes = await verifyConsolidationPaymentStatus(
                        bookingData.trackingNumber, 
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-5xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col md:flex-row h-[90vh] md:h-[80vh]">
                ...
                <div className="w-full md:w-5/12 bg-gradient-to-b from-gray-50 to-gray-100 p-6 md:p-10 border-b md:border-r border-gray-200 flex flex-col overflow-y-auto">
                    <div className="mb-8">
                        <h3 className="text-2xl font-black text-gray-900 tracking-tight">Consolidation Summary</h3>
                        <div className="flex items-center gap-2 mt-2">
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Original ID:</span>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 flex-1 flex flex-col relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-6 pb-4 border-b border-dashed border-gray-200">
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-xs font-bold text-amber-500 mt-1 bg-amber-50 px-2 py-0.5 rounded inline-block">Awaiting Fee</p>
                            </div>
                        </div>

                        <div className="space-y-4 mb-6">
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-gray-600 font-medium flex items-center gap-2"><FiCheckCircle className="text-green-500 w-4 h-4"/> Consolidation Fee</span>
                    ...
                    {stripeSession && stripeClientSecret ? (
                        <div className="flex flex-col h-full animate-in slide-in-from-right-4 duration-300">
                            <div className="flex items-center gap-3 mb-8">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-gray-500">Complete your consolidation fee via Stripe</p>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                                ...
                                >
                                    <StripePaymentForm 
                                        ...
                                        // ENSURING THE -CNSLDT- REFERENCE IS PASSED SO THE VERIFY PAGE SEES IT
                                        returnUrl={`${window.location.origin}/paymentstatus/stripe/verify?id=${bookingData.trackingNumber}&type=consolidation&reference=${stripeSession.transactionId}`}
                        ...
                        <div className="flex flex-col h-full overflow-hidden animate-in fade-in duration-300">
                            <div className="mb-8 pr-12">
                                ...
                                <p className="text-sm font-medium text-gray-500">Choose your preferred gateway to complete the consolidation fee securely.</p>
                            </div>
...
export default ConsolidationPaymentModal;